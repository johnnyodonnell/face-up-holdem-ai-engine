...
Loss = cross-entropy(policy) + MSE(value-vector). Standard AlphaZero objective
with the value loss summed across all NUM_SEATS output slots: the targets
contain a value for every seat (in the recording actor's mover frame), so
every slot supplies signal.

We don't mask logits on the network side — it outputs the full ACTION_SIZE
distribution and policy_target distributions sum to 1 over the legal subset,
which is all the cross-entropy needs.
...
Mirrors fox-lite-ai-engine/training/alphazero/train.py.
"""
...
def train_iteration(
    ...
    for _ in range(config.TRAIN_STEPS_PER_ITER):
        ...
        # MSE on per-seat value vector. Mean over batch AND seats.
